...
- Under hard phase shift, ML Set B improves hit rate from 74.4% to 96.7%.
- Under hard phase shift, ML Set B reduces object-store traffic from 1.94 GB to 0.23 GB.
- In the eviction stress test, admit-all + LRU reaches 96.7% hit rate with 0 evictions.
- In stationary hard load, LFU gets the highest hit rate among the simple baselines: 97.5% versus LRU at 72.5%.
- The offline ML model is modest but not dominant; Set B ROC AUC is 0.5529.
- In hard scale-event runs, Set C has the best hit rate among ML variants at 96.0%, but it also has high latency, indicating replacement churn is costly.
